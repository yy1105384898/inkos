...
import {
  deleteBrowserService,
  findBrowserService,
  saveBrowserService,
  setBrowserServiceSelection,
  type BrowserServiceConfig,
} from "../lib/browser-service-config";
...
export async function probeBrowserServiceForDetail(
  serviceId: string,
  body: {
    readonly apiKey: string;
    readonly apiFormat: "chat" | "responses";
  ...
  const fetchJsonImpl = deps?.fetchJsonImpl ?? fetchJson;
  return await fetchJsonImpl<ServiceProbeResponse>("/browser-services/test", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ service: serviceId, ...body }),
  });
}

export function readBrowserServiceForDetail(serviceId: string): BrowserServiceConfig | undefined {
  return findBrowserService(serviceId);
}

export async function saveBrowserServiceConfig(args: {
  ...
  if (!trimmedKey && !args.isCustom) {
    return { status: { state: "error", message: "请先输入 API Key" }, detectedModel: "", detectedConfig: null };
  }
  if (args.isCustom && !trimmedBaseUrl) {
    return { status: { state: "error", message: "请先填写 Base URL" }, detectedModel: "", detectedConfig: null };
  }
  ...
  try {
    probe = reuse && verified ? {
      ok: true,
      models: verified.models,
      selectedModel: verified.selectedModel,
      detected: verified.detected,
    } : await probeBrowserServiceForDetail(args.effectiveServiceId, {
      apiKey: trimmedKey,
      apiFormat: args.apiFormat,
      stream: args.stream,
      ...(args.isCustom ? { baseUrl: trimmedBaseUrl } : {}),
    }, { fetchJsonImpl: args.fetchJsonImpl });
  } catch (error) {
    return { status: { state: "error", message: error instanceof Error ? error.message : "连接失败" }, detectedModel: "", detectedConfig: null };
  }
  if (!probe.ok) {
    return { status: { state: "error", message: probe.error ?? "连接失败" }, detectedModel: "", detectedConfig: null };
  }
  ...
  const config: BrowserServiceConfig = {
    ...
    label: args.label,
    apiKey: trimmedKey,
    ...(args.isCustom ? { baseUrl: detectedConfig?.baseUrl ?? trimmedBaseUrl } : {}),
    apiFormat: detectedConfig?.apiFormat ?? args.apiFormat,
    stream: typeof detectedConfig?.stream === "boolean" ? detectedConfig.stream : args.stream,
    temperature: Number.parseFloat(args.temperature),
    ...(detectedModel ? { defaultModel: detectedModel } : {}),
  };
  saveBrowserService(config);
  if (detectedModel) {
    setBrowserServiceSelection({ service: config.service, model: detectedModel });
  }
  return {
...
export function deleteBrowserServiceConfig(serviceId: string): void {
  deleteBrowserService(serviceId);
}
