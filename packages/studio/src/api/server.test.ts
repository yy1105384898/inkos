...
const SERVICE_PRESETS_MOCK: Record<string, ServicePresetMock> = {
  ...
  yysubapi: { providerFamily: "openai", baseUrl: "https://yysubapi.yangyangnj.top/v1", modelsBaseUrl: "https://yysubapi.yangyangnj.top/v1", knownModels: [] as string[] },
...
const endpointIdsByGroup = {
  ...
  aggregator: ["yynewapi", "yysubapi", "openrouter", "siliconcloud"],
...
describe("createStudioServer daemon lifecycle", () => {
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          content: expect.arrayContaining([
            expect.objectContaining({
              text: expect.stringContaining("已为 demo-book 完成第 3 章"),
            }),
          ]),
        }),
      ]),
  ...
  it("binds an orphan session to activeBookId before running book tools", async () => {
    const orphanSession = {
      sessionId: "agent-session-1",
      bookId: null,
      title: null,
    ...
    const response = await app.request("http://localhost/api/v1/agent", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ instruction: "审计", activeBookId: "demo-book", sessionId: "agent-session-1" }),
    });

    expect(response.status).toBe(200);
    expect(migrateBookSessionMock).toHaveBeenCalledWith(root, "agent-session-1", "demo-book");
    const agentConfig = runAgentSessionMock.mock.calls.at(-1)?.[0] as Record<string, unknown>;
    expect(agentConfig.bookId).toBe("demo-book");
  });
